...
-(void)tabBar:(AXTabBar *)tabBar didSelectItem:(UITabBarItem *)item finishBlock:(void (^)(BOOL))success{

    NSInteger newSelectedIndex = [[tabBar items] indexOfObject:item];
    if (_selectedIndex != newSelectedIndex) {
        
        CGPoint oldOffset = _containerView.contentOffset ;
        
        [UIView animateWithDuration:0.3 animations:^{
            [_containerView setContentOffset:(CGPoint) {newSelectedIndex * CGRectGetWidth(_containerView.bounds), _containerView.contentOffset.y } animated:NO];
            [self changeSelectedIndex:newSelectedIndex];

        } completion:^(BOOL finished) {
            CGPoint newOffset = _containerView.contentOffset ;
