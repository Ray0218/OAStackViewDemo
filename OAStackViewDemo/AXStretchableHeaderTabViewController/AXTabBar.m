...
//        _indicatorView = [[UIImageView alloc]initWithImage:dp_SportLiveImage(@"liveArrow.png")];
...
//    [_indicatorView setFrame:(CGRect){CGRectGetMinX(button.frame), height - 7.0,
//        CGRectGetWidth(button.frame), 7.0}];
    [_indicatorView setCenter:(CGPoint){CGRectGetMidX(button.frame),height - 3.0
    }] ;
...
- (void)touchesButton:(UIButton *)sender {
    ...
    if (index != NSNotFound) {
        ...
        if (shouldSelectItem) {
            ...
            if ([_delegate respondsToSelector:@selector(tabBar:didSelectItem:finishBlock:)]) {
                [_delegate tabBar:self didSelectItem:selectedItem finishBlock:^(BOOL finish) {
                    if (finish) {
